...
def get_sequence_1_based(seq_dict, chrom, start, end, strand):
    """Returns the sequence from a genome sequence dictionary for a given chromosome, 1-based start and end positions, and strand."""
    # Check that chromosome is in the sequence dictionary
    if chrom not in seq_dict:
        raise ValueError(f"Chromosome {chrom} not found in seq_dict.")
    
    # Check that start < end 
    if start >= end and strand == "+":
        raise ValueError(f"Start position {start} is greater than or equal to end position {end} on the positive strand.")
    if start <= end and strand == "-":
        raise ValueError(f"End position {end} is greater than or equal to start position {start} on the negative strand.")
    
    # Get the sequence from the dictionary
    if strand == "+":
        sequence = seq_dict[chrom].seq[start-1:end-1]
    else: 
        sequence = seq_dict[chrom].seq[end:start].reverse_complement()
