...
def update_transcriptome_object(genome_path, transcriptome_path, output_filename, other_fields=None):
    """ Creates the transcriptome object and saves it to a file."""    

    # Generate the transcriptome object
    transcriptome_obj = generate_transcriptome_object(transcriptome_path, genome_path, other_fields)
...
def load_transcriptome_object(file_path):
    # Add .pkl to output_filename if it's not already there
    ...
    try:
        ...
        print(f"File {file_path} not found. Please run update(genome_path, transcriptome_path, output_filename) to generate the transcriptome object.")
        return None
